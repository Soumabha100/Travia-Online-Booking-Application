document.addEventListener("DOMContentLoaded", () => {
  ...
  // Security Layer to Check whether and User is Admin or Not !
  const adminBtn = document.getElementById("btnAdminPanel");
  if (user.isAdmin && adminBtn) {
    adminBtn.classList.remove("d-none"); // Reveal button only for Admins
  }
  ...
  if (idEl)
    idEl.textContent =
      "#" + (user.memberId || Math.floor(1000 + Math.random() * 9000));
